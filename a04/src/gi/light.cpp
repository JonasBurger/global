...
void SkyLight::commit() {
    ...
    for (size_t y = 0; y < tex->h; ++y) {
        ...
        float sin_theta = sinf(PI * float(y + .5f) / float(tex->h));
        for (size_t x = 0; x < tex->w; ++x)
            lum(x, y) = luma(tex->operator()(x, y)) * sin_theta;
    }
...
float SkyLight::pdf_Li(const SurfaceInteraction& light, const Ray& ray) const {
    // TODO ASSIGNMENT4 compute the PDF (solid angle) of the escaped ray
    // note: you may use Distribution2D::pdf
    const glm::vec2 spherical = to_spherical(ray.dir);
